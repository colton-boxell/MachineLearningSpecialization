...
def neutralize(word, g, word_to_vec_map):
    ...
    e_biascomponent = g * np.dot(e,g)/np.linalg.norm(g, ord=2)
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    e_w1B = bias_axis * np.dot(e_w1, bias_axis)/np.sum(bias_axis*bias_axis)
    e_w2B = bias_axis * np.dot(e_w2, bias_axis)/np.sum(bias_axis*bias_axis)
