...
def compute_content_cost(a_C, a_G):
    ...
    J_content = (1/(4*n_C*n_W*n_H))*tf.reduce_sum(tf.pow((a_C_unrolled - a_G_unrolled), 2))
    ### END CODE HERE ###
...
def compute_layer_style_cost(a_S, a_G):
    ...
    a_S = tf.transpose(tf.reshape(a_S, [n_H*n_W, n_C], name = None))
    a_G = tf.transpose(tf.reshape(a_G, [n_H*n_W, n_C], name = None))
    ...
    J_style_layer = (1/(4*n_C**2*(n_W*n_H)**2))*tf.reduce_sum(tf.pow((GS - GG), 2))
